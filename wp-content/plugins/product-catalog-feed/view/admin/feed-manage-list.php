...
class WpWoof_Feed_Manage_list extends Wpwoof_Feed_List_Table {
    ...
    function column_default($item, $column_name) {
        ...
        switch ($column_name) {
	        ...
	        case 'feednumber':
		        ...
		        $switcher = $isPro ? '' : '<div class="wpwoof-switch"><input ' . ( empty( $itemInfo['noGenAuto'] ) ? 'checked="true"' : '' ) . '" onchange="jQuery.fn.wpwoofSwicher(' . $option_id . ',jQuery(this).prop(\'checked\'));" type="checkbox" class="ios-switch toggleFeed" /></div>';
		        $feedPro = $isPro ? '<div class="wpwoof-feedpro"><p class="proFeedNotif">This feed was created in the PRO version and doesn\'t work with the FREE version.<br> Consider upgrading, or delete the feed.</p></div>' : '';
	...
	<div class="wpwoof-feedname">' . $itemInfo['feed_name'] . '</div>' . $switcher . $feedStatus . $feedPro . '
</div>';
...
//                trace($itemInfo,1);
		        ...
		        return  '<div class="wpwoof-feedname-content"><div class="wpwoof-loader" '.$loaders_style.'></div>&nbsp;' .  $addStr.$itemInfo['feed_name'] . '</div>'.$feedPro;
            case 'feed_type':
