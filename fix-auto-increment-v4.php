<?php
/**
 * Fix AUTO_INCREMENT v4 - Only fixes INTEGER-type ID columns.
 * Skips varchar/text IDs (like wp_mailchimp_carts).
 * DELETE AFTER USE!
 */
...
echo "=== Fix AUTO_INCREMENT v4 - INTEGER COLUMNS ONLY ===\n";
...
$fixed = 0; $skipped = 0; $errors = 0; $non_int = 0;
...
foreach ($all_tables as $table) {
    ...
    while ($col = $cols_res->fetch_assoc()) {
        ...
        // Only consider INTEGER types for auto-increment
        $is_int = (strpos($type, 'int') !== false || strpos($type, 'bigint') !== false ||
                   strpos($type, 'tinyint') !== false || strpos($type, 'smallint') !== false ||
                   strpos($type, 'mediumint') !== false);
        
        if (!$is_int) continue; // Skip non-integer columns
    ...
    // If no _id column found, check if first column is integer
    if (!$id_col && $first_col) {
        $ftype = strtolower($first_type);
        if (strpos($ftype, 'int') !== false || strpos($ftype, 'bigint') !== false ||
            strpos($ftype, 'tinyint') !== false || strpos($ftype, 'smallint') !== false ||
            strpos($ftype, 'mediumint') !== false) {
            $id_col = $first_col;
            $id_type = $first_type;
        }
    }
    ...
    if (!$id_col) {
        echo "  ⚠️ No integer ID column found, skipping\n\n";
        $skipped++;
        continue;
    }
